...
Subscribers.prototype = {
    ...
    // Always send e-mail to the default e-mail address specified in
    // config file.
    getMailingLists: function(callback) {
        if (config.email && config.email.mailinglists) {
            var lists = config.email.mailinglists;
            var emails = (lists instanceof Array) ? lists : [lists];
            var users = emails.map(function(email) { return {email: email, id: email}; });
            callback(null, users);
        }
        else {
            callback(null, []);
