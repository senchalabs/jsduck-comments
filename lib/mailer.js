...
Mailer.prototype = {
    ...
    batchSend: function(emails) {
        if (config.email.mode === "SES")
        {
          delete config.email.config['host'];
          delete config.email.config['port'];
        }
        this.transport = nodemailer.createTransport(config.email.mode, config.email.config);
    ...
    send: function(emails, callback) {
        ...
        this.transport.sendMail(mail, function(err,response) {
            if (err) {
                console.log("Failed sending e-mail to " + mail.to + ", Error: "+ err);
            }
            else {
                console.log("Sent e-mail to " + mail.to + ", " + response.message);
            }
